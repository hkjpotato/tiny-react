...
let now = function() {
 ...
 let cIC;
   ...
   const registeredCallbackIds = {}; // ok...so it is a queue then problem is about deletion?
   ...
   const frameDeadlineObject = {
     didTimeout: false,
     timeRemaining() {
       return  frameDeadline - now() ||  0;
     },
   };
 
   const safelyCallScheduledCallback = function(callback, callbackId) {
       // check if sth is being cancelled, not related to order
